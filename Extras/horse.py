...
def check_horse_winner(scores):
    l = len(scores)
    keepers = list((str(i), len(s)) for i, s in enumerate(scores) if len(s) < 5)
    n = len(keepers)
    if n == 1:
        return f"Player {keepers[0][0]} wins!"
    elif n != l:
        return f"Players {', '.join(k[0] for k in keepers)}: keep playing!"
    else:
